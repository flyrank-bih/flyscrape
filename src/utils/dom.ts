 ...
 */
export function cleanHtml(html: string | cheerio.CheerioAPI, options: CleanHtmlOptions = {}): string {
  const $ = typeof html === 'string' ? loadHtml(html) : html;
  const excludeTags = options.excludeTags || ['script', 'style', 'noscript', 'iframe', 'svg', 'link', 'meta'];
